...
public class BoardController {
    ...
    @ApiOperation(value = "글번호에 해당하는 게시글의 조회수를 증가한다. 그리고 DB수정 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@PutMapping("{articleno}")
	public ResponseEntity<String> countBoard(@PathVariable int articleno) {
		logger.debug("countBoard - 호출");
		logger.debug("" + articleno);
		
		if (boardService.countBoard(articleno)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
		return new ResponseEntity<String>(FAIL, HttpStatus.NO_CONTENT);
	}
